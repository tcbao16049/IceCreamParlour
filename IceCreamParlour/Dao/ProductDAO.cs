...
namespace ProjectSEM3.Dao
{
    ...
    public class ProductDAO
    {
        ...
        public IEnumerable<ProductModel> getAllProductProducer(ref int totalRecord, int pageIndex, int pageSize, int id, int cateId)
        {
            ...
            foreach (var item in productcream)
            {
                var images = "";
                images = item.MoreImage;
                if (images != null)
                {
                    ...
                    foreach (XElement element in xImages.Elements())
                    {
                        var product = productcream.First(x => x.ID == item.ID).MoreImage = element.Value;
                    }
                }
            }

            totalRecord = productcream.Count(); //lấy tổng số lượng sản phẩm 
            var models = productcream.Skip((pageIndex - 1) * pageSize).Take(pageSize);
            return models;
